...
const FALLBACK_NGO_IMAGES = [
  "https://images.unsplash.com/photo-1509099836639-18ba1795216d?w=400&h=300&fit=crop",
  "https://images.unsplash.com/photo-1473649085228-583485e6e4d7?w=400&h=300&fit=crop",
  "https://images.unsplash.com/photo-1521493959102-bdd6677fdd81?w=400&h=300&fit=crop",
  "https://images.unsplash.com/photo-1543689604-6fe8dbcd1f59?w=400&h=300&fit=crop",
];

const FALLBACK_OPP_IMAGES = [
  "https://images.unsplash.com/photo-1567057419565-4349c49d8a04?w=600&h=400&fit=crop",
  "https://images.unsplash.com/photo-1528502668750-88ba58015b2f?w=600&h=400&fit=crop",
  "https://images.unsplash.com/photo-1617056239820-8ce90ba48193?w=600&h=400&fit=crop",
];
...
function Home() {
  ...
  const [liveCounts, setLiveCounts] = useState({ volunteers: 0, ngos: 0, opportunities: 0 });
  const [volCount, volRef] = useCountUp(liveCounts.volunteers);
  const [ngoCount, ngoRef] = useCountUp(liveCounts.ngos);
  const [hrCount, hrRef] = useCountUp(liveCounts.opportunities);

  // Live data from Supabase — falls back to curated sample content only if
  // the platform genuinely has nothing to show yet, so the homepage never
  // displays fabricated numbers alongside real ones.
  useEffect(() => {
    let isMounted = true;

    const loadHomeData = async () => {
      const [{ count: volunteerCount }, { count: ngoCount }, { count: oppCount }] = await Promise.all([
      ...
      const { data: ngoRows } = await supabase
        .from("ngos")
        .select("*")
        .eq("approval_status", "approved")
        .limit(4);
      if (isMounted) {
        setNgos(ngoRows && ngoRows.length > 0 ? ngoRows : staticNgos);
      ...
      const { data: oppRows } = await supabase
        ...
        .limit(3);
      if (isMounted) {
        setOpportunities(oppRows && oppRows.length > 0 ? oppRows : staticOpps);
      ...
      <section className="hero-section" ref={heroRef}>
        <div className="hero-left">
          <div className="hero-badge-top">
            <span className="hero-badge-dot" /> Trusted by {liveCounts.ngos > 0 ? `${liveCounts.ngos}+` : "growing"} NGOs across Pakistan
          </div>
        ...
        <div className="hero-visual">
          <div className="hero-card">
            <img
              ...
              alt="Volunteers supporting children in the community"
              className="hero-img"
            />
      ...
      <section className="stats-bar" ref={statsRef}>
        <div className="stats-bar-inner">
          ...
          <div className="stat-bar-item" ref={hrRef}>
            ...
            <div className="stat-bar-label">Opportunities Posted</div>
          </div>
        </div>
      </section>
            ...
            ngos.map((ngo, i) => (
              <Link to={`/ngos/${ngo.id}`} key={ngo.id} className="ngo-home-card">
                <div className="ngo-home-img-wrap">
                  <img src={ngo.image || FALLBACK_NGO_IMAGES[i % FALLBACK_NGO_IMAGES.length]} alt={ngo.name} loading="lazy" />
                  <div className="ngo-home-img-overlay" />
                </div>
                <div className="ngo-home-body">
                  <span className="ngo-home-category">{ngo.category || "General"}</span>
                  <h4>{ngo.name}</h4>
                  <div className="ngo-home-location"><MapPinIcon /> {ngo.city || ngo.address || ngo.location || "Pakistan"}</div>
                </div>
              </Link>
            ))
          )}
            ...
            opportunities.map((opp, i) => (
              <div key={opp.id} className="opp-home-card">
                <div className="opp-home-visual">
                  <img src={opp.image || FALLBACK_OPP_IMAGES[i % FALLBACK_OPP_IMAGES.length]} alt={opp.title} loading="lazy" />
                  <div className="opp-home-img-overlay" />
                  <span className="opp-home-type">{opp.type || "Volunteer"}</span>
                </div>
                <div className="opp-home-body">
                  <span className="opp-home-category">{opp.category || "General"}</span>
                  <h4>{opp.title}</h4>
                  <p>{opp.description}</p>
                  <div className="opp-home-meta">
                    <span><MapPinIcon /> {opp.location || "Remote"}</span>
                    <span><ClockIcon /> {opp.type || "Flexible"}</span>
                  </div>
                  <Link to={opp.id ? `/opportunity/${opp.id}` : "/opportunities"} className="opp-home-link">
