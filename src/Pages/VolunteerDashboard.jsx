...
import {
  ClipboardList, CheckCircle2, Clock, Timer, Award, FileWarning, Search, BellRing,
} from "lucide-react";
import { PieChart, Pie, Cell, ResponsiveContainer, Tooltip } from "recharts";
import StatCard from "../components/StatCard";
import "./VolunteerDashboard.css";

const APP_CHART_COLORS = { Approved: "#2d6a4f", Pending: "#e8a33d", Rejected: "#dc2626" };
...
<div class="cert-footer"><div class="cert-date">Date: ${new Date().toLocaleDateString()}</div><div class="cert-logo">🌿 NGO Connect</div></div>
  ...
  return (
    <div style={{ background: "#F7F5EF", minHeight: "100vh", fontFamily: "'Inter', sans-serif" }}>
                ...
                <div style={{ maxHeight: 360, overflowY: "auto" }}>
                  {notifications.length === 0 ? (
                    <div style={{ padding: "28px", textAlign: "center", color: "#8A8F86", fontSize: "13px" }}>
                      <BellRing size={14} style={{ verticalAlign: "-2px", marginRight: 6 }} /> No notifications yet
                    </div>
                  ) : (
                    notifications.map((n) => (
                      <div key={n.id} style={{
      ...
      {/* Stats Cards */}
      <div style={{ padding: "24px 32px 0" }} className="stat-cards-row">
        <StatCard icon={ClipboardList} value={applications.length} label="Total Applications" tone="green" />
        <StatCard icon={CheckCircle2} value={approvedCount} label="Approved" tone="green" />
        <StatCard icon={Clock} value={pendingCount} label="Pending" tone="amber" />
        <StatCard icon={Timer} value={hoursLogged} label="Hours Logged" tone="blue" />
        <StatCard icon={Award} value={certificatesEarned} label="Certificates" tone="coral" />
        <StatCard icon={FileWarning} value={myComplaints.length} label="Reports Filed" tone="amber" />
      </div>

      {/* Main Grid */}
      <div style={{ padding: "24px 32px 40px", display: "grid", gridTemplateColumns: "1fr 340px", gap: "24px" }}>
        {/* LEFT COLUMN */}
        <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          {/* Application Status Breakdown */}
          {applications.length > 0 && (
            <div style={{ background: "white", borderRadius: "16px", border: "1px solid #E4E0D6", padding: "20px 24px" }}>
              <h2 style={{ margin: "0 0 12px", fontSize: "17px", color: "#1B3A28", fontFamily: "'Fraunces', serif" }}>Your Application Progress</h2>
              <div style={{ display: "flex", alignItems: "center", gap: "24px", flexWrap: "wrap" }}>
                    ...
                    <PieChart>
                      <Pie
                        data={[
                          ...
                          { name: "Rejected", value: applications.length - approvedCount - pendingCount },
                        ].filter(d => d.value > 0)}
                      ...
                      >
                        {[
                          ...
                          <Cell key={d.name} fill={APP_CHART_COLORS[d.name]} />
                        ))}
                      </Pie>
                      <Tooltip />
                    </PieChart>
                ...
                <div style={{ flex: 1, minWidth: 180 }}>
                  <div style={{ fontSize: "13px", color: "#6B7268", marginBottom: "10px", fontWeight: 500 }}>
                    Impact score: <strong style={{ color: "#1B3A28" }}>{Math.round(impactPercent)}%</strong>
                  </div>
                  <div style={{ background: "#F0EDE6", borderRadius: 999, height: 10, overflow: "hidden" }}>
                    <div style={{ background: "linear-gradient(90deg,#2d6a4f,#40916c)", width: `${impactPercent}%`, height: "100%", borderRadius: 999 }} />
                  </div>
                  <div style={{ display: "flex", gap: "14px", marginTop: "14px", flexWrap: "wrap" }}>
                    {[
                      ...
                      <div key={d.name} style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "12.5px", color: "#4A4A4A", fontWeight: 500 }}>
                        <span style={{ width: 9, height: 9, borderRadius: "50%", background: APP_CHART_COLORS[d.name] }} />
                        {d.name} · {d.value}
                      </div>
                    ))}
                  </div>
          ...
          <div style={{ background: "white", borderRadius: "16px", border: "1px solid #E4E0D6", overflow: "hidden" }}>
            ...
            {filteredApps.length === 0 ? (
              <div style={{ padding: "48px 24px", textAlign: "center" }}>
                <Search size={40} color="#C7C2B4" style={{ marginBottom: "12px" }} />
