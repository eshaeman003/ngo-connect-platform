...
import {
  Landmark, Users, Hourglass, ClipboardList, Megaphone, ShieldAlert,
} from "lucide-react";
import { PieChart, Pie, Cell, ResponsiveContainer, Tooltip, BarChart, Bar, XAxis, YAxis, CartesianGrid, Legend } from "recharts";
import "./AdminDashboard.css";

const NGO_STATUS_COLORS = { Approved: "#2d6a4f", Pending: "#e8a33d", Rejected: "#dc2626" };
...
function AdminDashboard() {
  ...
  const statCards = [
    { key: "approvedNGOs", label: "Approved NGOs", icon: Landmark, color: "moss", tab: "approved" },
    { key: "totalVolunteers", label: "Volunteers", icon: Users, color: "ink", tab: "volunteers" },
    { key: "pendingApprovals", label: "Pending NGOs", icon: Hourglass, color: "gold", tab: "ngos" },
    { key: "totalApplications", label: "Applications", icon: ClipboardList, color: "rust", tab: "applications" },
    { key: "totalOpportunities", label: "Opportunities", icon: Megaphone, color: "blue", tab: "opportunities" },
    { key: "pendingComplaints", label: "Complaints", icon: ShieldAlert, color: "danger", tab: "complaints", nav: "/admin/complaints" },
  ];

  const ngoStatusChart = [
    { name: "Approved", value: approvedNGOsList.length },
    { name: "Pending", value: pendingNGOs.length },
    { name: "Rejected", value: rejectedNGOsList.length },
  ].filter(d => d.value > 0);
    ...
    <div className="admin-page">
      ...
      <div className="admin-stats">
        {statCards.map((s) => (
          <div key={s.key} className={"admin-stat " + s.color + (flashStat === s.key ? " flash" : "")} onClick={() => s.nav ? navigate(s.nav) : setActiveTab(s.tab)} style={{ cursor: "pointer" }} title={"View " + s.label}>
            ...
            <div className="stat-top"><span className="stat-num">{stats[s.key]}</span><span className="stat-icon"><s.icon size={22} strokeWidth={2} /></span></div>
      ...
      <div className="admin-insights">
        <div className="insight-card">
          <h3>NGO Approval Status</h3>
          {ngoStatusChart.length === 0 ? (
            <p className="insight-empty">No NGOs registered yet.</p>
          ) : (
            <div className="insight-donut-row">
              <div className="insight-donut">
                <ResponsiveContainer width="100%" height={180}>
                  <PieChart>
                    <Pie data={ngoStatusChart} dataKey="value" nameKey="name" innerRadius={50} outerRadius={75} paddingAngle={3}>
                      {ngoStatusChart.map((d) => <Cell key={d.name} fill={NGO_STATUS_COLORS[d.name]} />)}
                    </Pie>
                    <Tooltip />
                  </PieChart>
                </ResponsiveContainer>
              ...
              <div className="insight-legend">
                {ngoStatusChart.map(d => (
                  <div className="insight-legend-item" key={d.name}>
                    <span className="insight-dot" style={{ background: NGO_STATUS_COLORS[d.name] }} />
                    {d.name} · {d.value}
                    <span className="insight-legend-pct">
                      {Math.round((d.value / ngos.length) * 100)}%
                    </span>
                  </div>
                ))}
              </div>
        ...
        <div className="insight-card">
          ...
          <ResponsiveContainer width="100%" height={220}>
            <BarChart data={activityBarData} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
